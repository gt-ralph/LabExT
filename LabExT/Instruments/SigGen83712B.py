...
class SigGen83712B(Instrument):
    ...
    * **get_data**: downloads the wavelength and power data of the last measurement
    ...
    def open(self):
        ...
        authentication = self._inst.query('OUTP:STATE?')
        print(authentication)

        if authentication.strip() == "+0":
            self.command("OUTP:STATE 1")
            time.sleep(0.2)
            authentication_post_init = self._inst.query('OUTP:STATE?')
...
#for now
    ...
    def set_output(self, state = 0):
        ...
        :return: none
        """
        self.command(f':OUTP:STATE {state}')
        return
